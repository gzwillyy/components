...
var (
	unknownCoder defaultCoder = defaultCoder{1, http.StatusInternalServerError, "An internal server error occurred", ""} // http://github.com/gzwillyy/components/errors/README.md
)
...
func Register(coder Coder) {
	if coder.Code() == 0 {
		panic("code `0` is reserved by `errors pkg` as unknownCode error code")
	}
...
func MustRegister(coder Coder) {
	if coder.Code() == 0 {
		panic("code '0' is reserved by 'errors pkg' as ErrUnknown error code")
	}
